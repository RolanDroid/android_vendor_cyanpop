...
ifdef CYANPOP_BUILDTYPE
    ifeq ($(CYANPOP_BUILDTYPE), RELEASE)
       CYANPOP_VERSION := $(TARGET_PRODUCT)-$(VERSION)-RELEASE-$(shell date -u +%Y%m%d)
    endif
    ifeq ($(CYANPOP_BUILDTYPE), NIGHTLY)
        CYANPOP_VERSION := $(TARGET_PRODUCT)-$(VERSION)-NIGHTLY-$(shell date -u +%Y%m%d)
    endif
    ifeq ($(CYANPOP_BUILDTYPE), EXPERIMENTAL)
        CYANPOP_VERSION := $(TARGET_PRODUCT)-$(VERSION)-EXPERIMENTAL-$(shell date -u +%Y%m%d)
    endif
    ifeq ($(CYANPOP_BUILDTYPE), UNOFFICIAL)
        CYANPOP_VERSION := $(TARGET_PRODUCT)-$(VERSION)-UNOFFICIAL-$(shell date -u +%Y%m%d)
    endif
else
        CYANPOP_VERSION := $(TARGET_PRODUCT)-$(VERSION)-UNOFFICIAL-$(shell date -u +%Y%m%d)
endif

CYANPOP_DISPLAY_VERSION := $(CYANPOP_VERSION)
